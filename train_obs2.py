...
# Description:  
# LastEditors: Shiyuec
# LastEditTime: 2024-11-20 16:56:51
## 
##
# Author:  
# Description:  
# LastEditors: Shiyuec
# LastEditTime: 2024-11-20 16:54:40
## 
import torch
...
class ObservationToStateModel(nn.Module):
    def __init__(self, input_size, hidden_size=128, output_size=None):
        ...
        if output_size is None:
            output_size = encoder.num_states
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=0.5)
        self.fc2 = nn.Linear(hidden_size, output_size)
        self.softmax = nn.Softmax(dim=1)  # 预测类别分布概率
...
model = ObservationToStateModel(input_dim, output_dim)
...
def get_topk_state(predictions):
    ...
    for  k in top_k_output[0]:
        ...
        print(f"高置信度预测: 状态 {state}，概率 = {predictions[0][k]:.4f}")
