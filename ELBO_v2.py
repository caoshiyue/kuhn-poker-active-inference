...
# Description:  
# LastEditors: Shiyuec
# LastEditTime: 2024-11-22 10:46:48
## 
##
# Author:  
# Description:  
...
def compute_elbo_dirichlet(model, agent, obs_batch, action_batch, prior_alpha=1.0):
    """
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        for batch_obs, batch_actions in tqdm(dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            log_p_data += torch.log(action_probs[0][ba.item()] + 1e-10)
        ...
        elbo = log_p_data + log_p_theta - log_q_theta
        loss = -elbo
        print(f"log_p_data: {log_p_data.item()}, log_p_theta: {log_p_theta.item()}, log_q_theta: {log_q_theta.item()}")
